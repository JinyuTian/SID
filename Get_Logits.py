...
parser.set_defaults(FixedWave = 'Trainable')
parser.set_defaults(AdvNoise=0.05)
parser.set_defaults(Calibration=0)
best_prec1 = 0

DIR = {'F':'FGSM','F1':'FGSM_0.1','F2':'FGSM_0.2','F3':'FGSM_0.3','F4':'FGSM_0.4',
       'B':'BIM','B2':'BIM-b_0.2','DF':'DeepFool','C':'CWL2',
       'CLF3':'C&W_Linf_0.3','CLF':'C&W_Linf'}
TESTDIR = ['FGSM_0.01','FGSM_0.05','FGSM_0.1','BIM-b_0.1','DeepFool_0.05','DeepFool_0.18','C&W_Linf_0.1']

def main():
    global args, DIR,TESTDIRDIR
    args = parser.parse_args()
    adv_test_list, adv_noisy_list = get_adv_noisy(args.net_type, args.dataset)
    for i,(F,A) in enumerate(zip(adv_test_list,adv_noisy_list)):
        if i == 1:
            break
        Get_Logits(F,A,args)

def Get_Logits(F,A,args):
    ...
    if args.dataset == 'imagenet':
        ...
        args.num_class = 10
    args.PDresume = './pre_trained/' + 'PD' + args.net_type + '_' + args.dataset + '.pth'
    args.FDresume = './pre_trained/' + 'FD' + args.net_type + '_' + args.dataset + '.pth'

    #### Create PD and FD models
    args.domain = 'PD'
    PDmodel = GET_MODEL(args)
    args.domain = 'FD'
    FDmodel = GET_MODEL(args)
    ...
    #### Generate Data
    print('################## Load Training Adv Data: {}_{}_{} ##################'.format(args.dataset,args.net_type,args.AdvNoise))
    clean_data, adv_data, noisy_data, label = get_data(args)
    CPFLogits, CPLogits, CLabel, CPF_Targets, CP_Targets = GetLogitValues(clean_data, label, PDmodel, FDmodel, args,
                                                                          Natural=True)
    NsyPFLogits, NsyPLogits, NsyLabel, NsyPF_Targets, NsyP_Targets = GetLogitValues(noisy_data, label, PDmodel,
                                                                                    FDmodel, args, Natural=True)
    AdvPFLogits, AdvLabel, AdvPF_Targets = GetLogitValues(adv_data, label, PDmodel, FDmodel, args, Natural=False)
...
def GetLogitValues(Data, label, PDmodel, FDmodel, args,Natural=True):
    ...
    while not len(TrainIndex) == 0:
        if len(TrainIndex) < args.TB:
            inputs = torch.from_numpy(Data[TrainIndex[0:]])
            target = torch.from_numpy(label[TrainIndex[0:]])
        else:
            inputs = torch.from_numpy(Data[TrainIndex[0:args.TB]])
            target = torch.from_numpy(label[TrainIndex[0:args.TB]])
        ...
        if Natural == True:
            ...
            TPFLogits[1,:] = FDx
            PFLogits = np.concatenate((PFLogits,TPFLogits),axis=1)
            PF_Targets = np.concatenate((PF_Targets, target.numpy()))
        if i % args.print_freq == 0:
    ...
    if Natural == True:
        ...
        return PFLogits, PLogits, Label, PF_Targets, P_Targets
    else:
        PFLogits = PFLogits[:,2:, ]
        PF_Targets = PF_Targets[2:]
        Label = 2 * np.ones(PFLogits.shape[1])
        return PFLogits, Label, PF_Targets
...
def get_data(args):
    clean_data = torch.load(
        '{0}/{1}_{2}/clean_data_{1}_{2}_{3}_{4:.2f}.pth'.format(args.outf, args.net_type, args.dataset, args.adv_type,
                                                                args.AdvNoise))
    adv_data = torch.load(
        '{0}/{1}_{2}/adv_data_{1}_{2}_{3}_{4:.2f}.pth'.format(args.outf, args.net_type, args.dataset, args.adv_type,
                                                              args.AdvNoise))
    noisy_data = torch.load(
        '{0}/{1}_{2}/noisy_data_{1}_{2}_{3}_{4:.2f}.pth'.format(args.outf, args.net_type, args.dataset, args.adv_type,
                                                                args.AdvNoise))
    label = torch.load(
        '{0}/{1}_{2}/label_{1}_{2}_{3}_{4:.2f}.pth'.format(args.outf, args.net_type, args.dataset, args.adv_type,
                                                           args.AdvNoise))
...
def GET_MODEL(args):
    args.resume = './pre_trained/' + args.domain+args.net_type + '_' + args.dataset + '.pth'
...
def get_adv_noisy(net_type,dataset):
    if net_type == 'VggBn' and dataset == 'cifar10':
        adv_test_list = ['DeepFool','DeepFool','DeepFool','CWL2','FGSM','FGSM','FGSM','BIM','BIM','BIM']
        adv_noisy_list = [1.35,0.73,0.41,0.15,0.25,0.05,0.03,0.26,0.13,0.05]
    if net_type == 'VggBn' and dataset == 'svhn':
        adv_test_list = ['DeepFool', 'CWL2', 'FGSM', 'FGSM', 'BIM', 'BIM']
        adv_noisy_list = [0.42, 0.12, 0.5, 0.25, 0.26, 0.13]
    if net_type == 'resnet' and dataset == 'cifar10':
        adv_test_list = ['DeepFool', 'DeepFool', 'CWL2', 'FGSM', 'FGSM', 'FGSM', 'BIM', 'BIM', 'BIM']
        adv_noisy_list = [0.36, 0.17, 0.15, 0.25, 0.05, 0.03, 0.26, 0.13, 0.05]
    if net_type == 'resnet' and dataset == 'svhn':
        adv_test_list = ['DeepFool', 'DeepFool', 'CWL2', 'FGSM', 'FGSM', 'BIM', 'BIM', 'BIM']
        adv_noisy_list = [1.09, 0.62, 0.14, 0.25, 0.05, 0.26, 0.13, 0.05]
